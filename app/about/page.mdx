...
export const aboutGallery = [
  ...
  },
  {
    ...
    description: "The setup with laptop, pi, tv and Suki ☁️",
  },
  {
    type: "video",
...
Naturally, the theme is **dog photos**. Because dogs are awesome. 🐶
...
They send data back and forth using IP addresses, URLs, and something called DNS (Domain Name System)—often referred to as the internet's "phone book."
...
- Figure out why my Pi kept disconnecting from the internet 🫠

There's a fair bit more going on behind the scenes—like a database, an AI-powered DogBot, and a system profiler—but we'll get into that next. That's all just software I wrote which runs on my Pi.

### Bonus: What makes a computer a server?

Any computer can be a server—even yours! All it takes is downloading some free server software like [nginx](https://nginx.org/) or [Apache](https://httpd.apache.org/) and turning it on.
...
I began with [this tutorial](https://www.youtube.com/watch?v=Vp4glSVPT8o), which got me a basic fullstack [Next.js](https://nextjs.org/) app running on the Pi, with traffic routed through a [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/). That was a big win right away, so I just kept going.

Initially, the app didn't have a database, authentication, or any of that fancy stuff. I was just adding features as I went along without any real plan.
...
Setting up docker was a challenge. I thought it was supposed to make my life easier?!?! Well, turns out I was just doing it wrong. For example, I was rebuilding the images for DogBot on every build, which was obviously super slow because of the ai models... 🤦‍♂️
...
- Prisma ➡️ [Drizzle](https://orm.drizzle.team/)
- SQLite ➡️ [PostgreSQL](https://www.postgresql.org/)
...
At this point, the app was feature-complete, so all I had to do was add some more features! 🤷‍♂️

#### MDX

I added support for [MDX](https://mdxjs.com/) files, which allowed me to write this /about page in markdown, but still use react components when I needed to. I used another tutorial from Lee Rob, [MDX with Next.js App Router](https://www.youtube.com/watch?v=34bRv6cQezo), to help me set it up. This was my first time using MDX, and I'm in love! Definitely going to use it on more projects in the future.

#### Yet Another React Lightbox

I used [Yet Another React Lightbox](https://yet-another-react-lightbox.com/) for the fullscreen image and video previews. It's great! I integrated it to use Next.js Images which come with extra features for optimizing images for the web.

#### The Logo

As with most of my projects, I designed the DogTown logo using Figma. I really love creating logos and giving my projects some personality.
...
While writing this page, I realized I kind of skipped over any mention of my experience with the core tech: Next.js, React, Tailwind, and ShadCN. Basically, these have been my daily drivers for a while now, so I didn't feel the need to talk about them. They're great, as always, thanks for asking!

One new thing that I tried here was using [V0](https://v0.dev/) to generate a component library for a "dog themed" app and then selectively pulling out comopnents and styles that I liked. It's a great way to get some fresh design ideas. For example, that's what inspired the navbar design.

---

## Final Thoughts

I learned a ton from this project. I got to try a lot of technologies that I'd never used before, and dive deep into some others. I went through multiple iterations of the app with different tech stacks, and I think I arrived on a stack that I'd be happy using for future projects. I've never been one to use "starter templates" for my projects, but I think this could be a good one.

### Wishlist

Here's what I'd like to add if I had unlimited time:

- More CRUD operations: to make it more of a full-featured app.
- Better type safety: with [Zod](https://zod.dev/). There's a [video by CJ at SyntaxFM](https://www.youtube.com/watch?v=sYZW8TK2IV4), where he shows off a super clean Hono/Zod setup.
- Dev Container: because I've come a long way with Docker.
- Better charts: the ReCharts library I used is great, but I don't love the way they animate when they receive new data.

p.s. I know the whole "\_\_ for dogs" idea has been done a thousand times, but idc. I'll make dog-themed apps until they stop bringing me joy!
