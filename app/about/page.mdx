...
import { VideoWithCaption } from "@/components/video-with-caption";
...
export const aboutGallery = [
  {
    src: "/images/My-Pi.jpeg",
    alt: "A Raspberry Pi",
    description: "The Raspberry Pi powering this site",
  },
  {
    src: "/images/Tech-For-Dummies.jpg",
    alt: "My Raspberry Pi setup",
    description: "My Raspberry Pi setup with all the essentials",
  },
  {
    src: "/images/Tech-Stack.jpg",
    alt: "Tech stack diagram",
    description: "The tech stack powering this site",
  },
  {
    type: "video",
    sources: [{ src: "/videos/DogBot-Demo.mp4", type: "video/mp4" }],
    description: "Hammers are not dogs. Good to know!",
  },
  {
    src: "/images/The-Setup-OG.jpeg",
    alt: "My Raspberry Pi setup",
    description: "The physical setup with keyboard, mouse, and monitor",
  },
];
...
While not built for gaming, Raspberry Pis are fantastic for DIY projects and experimentation. You can create robots, smart home devices—or even host a website. Which is exactly what I did.
...
I've always used cloud services like Vercel and Netlify to host my projects. They're fast, simple, and take care of a lot behind the scenes.

But for this one, I wanted a challenge—something that would push me to understand self-hosting, fill knowledge gaps, and get hands-on with my new Pi.
...
<div className="not-prose">
  <LightboxImage
    ...
    width={600}
    height={400}
    caption="My Raspberry Pi setup with all the essentials"
    enableLightbox
    gallery={aboutGallery}
    galleryIndex={1}
  />
...
There's more going on behind the scenes—like a database, an AI-powered DogBot, and a system profiler—but we'll get into that next.

### Bonus: What makes a computer a server?

Any computer can be a server—even yours! All it takes is downloading some free server software.
...
<div className="not-prose">
  <LightboxImage
    ...
    width={600}
    height={400}
    caption="The tech stack powering this site"
    enableLightbox
    gallery={aboutGallery}
    galleryIndex={2}
  />
...
Initially, the app used Node.js and the filesystem for storage—photos and metadata saved as JSON. I also had a simple UUID tagging system instead of real auth. Functional, but definitely not scalable.
...
- Wrote a Bash script to update the app and restart the server—my first step into DevOps.
...
The service runs on a **FastAPI** server with one endpoint. It receives an image and returns `isDog` and `isNSFW` booleans. I tested it with existing site images—everything passed.
...
<VideoWithCaption
  src="/videos/DogBot-Demo.mp4"
  caption="Hammers are not dogs. Good to know!"
...
Docker was a pain at first. Builds failed constantly. But once I discovered [Lee Rob's tutorial](https://www.youtube.com/watch?v=sIVL4JMqRfc), things clicked.
...
There's even a mock mode using generator functions and sample data for local development on macOS.
...
<div className="not-prose">
  <LightboxImage
    ...
    width={600}
    height={400}
    caption="The physical setup with keyboard, mouse, and monitor"
    enableLightbox
    gallery={aboutGallery}
    galleryIndex={4}
  />
...
- **AI Validation:** Every photo is checked for NSFW content and to ensure it's actually a dog before it's uploaded. Keeps things clean and on-brand.

- **Live Monitoring:** A real-time dashboard displays CPU, memory, network, and container health—powered by the system-profiler service and streamed via SSE.
