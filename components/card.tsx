...
const CardHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex flex-row items-center justify-start space-y-1.5 p-6 md:block", className)}
      {...props}
    />
  )
);
...
const CardIcon = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "mb-0 mr-4 h-12 w-12 cursor-pointer rounded-2xl border border-slate-100 bg-transparent p-2 text-6xl text-card-foreground hover:border-slate-200 hover:bg-slate-100 md:static md:mb-8 md:mr-0",
        className
      )}
      {...props}
    />
